...
class UsersController < ApplicationController
  ...
  def create
    # @quantity_allowed = CompanyConfiguration.where(company_id:  params[:id_company]).last.quantity_admins
    # @quantity_registered = User.where("company_id = ? and permission_level = ?",params[:id_company], 3).count
    #
